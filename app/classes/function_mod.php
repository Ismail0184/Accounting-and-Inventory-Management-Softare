...
$sql="select
p.main_menu_id as powerpage,
p.user_id,
p.powerby,
dmm.main_menu_id as page_id,
dmm.main_menu_name,
dmm.url as pageurl
from
user_permission_matrix_main_menu p,
dev_main_menu dmm
where
...
function check_permission($page){
	global $conn;
	$page_id_GET=find_a_field('dev_sub_menu','sub_menu_id','`sub_url` LIKE "'.$page.'"');
	$sql="select
	p.sub_menu_id as powerpage,
	p.main_menu_id,
	p.user_id,
	p.powerby,
	z.sub_menu_id as page_id,
	z.sub_menu_name,
	z.sub_url as pageurl
	from
	user_permission_matrix_sub_menu p,
	dev_sub_menu z
	where
...
function add_to_payment ($initiate_credit_note,$date,$proj_id,$narration,$ledger_id,$dr_amt,$cr_amt,$type,$cur_bal,$paid_to,
                         $Cheque_No,$c_date,$Cheque_of_bank,$manual_payment_no,$cc_code,$subledger_id,$status,$ip,$voucher_date,$sectionid,$companyid,$userid,$create_date,$now,$day
    ,$thisday,$thismonth,$thisyear,$receive_ledger,$lc_id)
{
	global $conn;
 $recept="INSERT INTO `payment` (
							...
							'$sectionid',
							'$companyid',
							'$userid','','$create_date','$now','$day','$thisday','$thismonth','$thisyear','$lc_id')";
 $query_receipt = mysqli_query($conn, $recept);
}
//// /receipt add data update
...
function add_to_bank_payment ($initiate_credit_note,$date,$proj_id,$narration,$ledger_id,$dr_amt,$cr_amt,$type,$cur_bal,$paid_to,
                         $cheque_id,$maturity_date,$Cheque_of_bank,$manual_payment_no,$cc_code,$subledger_id,$status,$ip,$voucher_date,$sectionid,$companyid,$userid,$create_date,$now,$day
    ,$thisday,$thismonth,$thisyear,$receive_ledger)
{ global $conn;
...
function approximate_ledger_id($group_id)
{ global $conn;
...
function sub_ledger_generate($sub_ledger_id,$name, $under, $balance, $now, $proj_id)
{ global $conn;
...
function sub_sub_ledger_generate($sub_ledger_id,$name, $under, $balance, $now, $proj_id)
{ global $conn;
...
function ledger_generate($ledger_id,$ledger_name,$ledger_group_id,$opening_balance,$balance_type,$depreciation_rate,$credit_limit, $opening_balance_on,$proj_id,$budget_enable='NO',$parent)
{
 global $conn;
 ...
 $sql="INSERT INTO `accounts_ledger`
		...
		`proj_id`,
		`budget_enable`,
		group_for,section_id,company_id,entry_at,entry_by,parent)
		VALUES ('$ledger_id','$ledger_name', '$ledger_group', '$opening_balance', '$balance_type', '$depreciation_rate', '$credit_limit', '$opening_balance_on','$proj_id','$budget_enable','".$_SESSION['usergroup']."','".$_SESSION['sectionid']."','".$_SESSION['companyid']."','".date('Y-m-d H:i:s')."','".$_SESSION['userid']."','$parent')";
...
function group_excess($group_name,$manual_group_code='',$group_id='')
{ global $conn;
