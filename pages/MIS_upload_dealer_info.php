...
$page="MIS_upload_dealer_info.php";
$crud      =new crud($table);
$source="item_info";
...
if(isset($_POST['clearAll']))
    ...
    { $query = "INSERT INTO ".$table." (`dealer_code`,`dealer_custom_code`,`dealer_name_e`,`propritor_name_e`,`dealer_category`,`territory`,`town_code`,`area_code`,`region`,`section_id`,`company_id`,`mobile_no`,`address_e`,`account_code`)
          SELECT dealer_code,dealer_custom_code,dealer_name_e,propritor_name_e,outlet_category,territory,town_code,area_code,region,section_id,company_id,mobile_no,address_e,ledger_id FROM data_upload_helper
          WHERE 1";
        $insert=mysqli_query($conn, $query);

        $query1 = "INSERT INTO accounts_ledger (`ledger_id`,`ledger_name`,`ledger_group_id`,`section_id`,`company_id`)
          SELECT ledger_id, dealer_name_e,ledger_group_id,section_id,company_id FROM data_upload_helper
          WHERE 1";
        //$insert1=mysqli_query($conn, $query1);

        $query2 = "INSERT INTO sub_ledger (`ledger_id`,`sub_ledger_id`,`sub_ledger`,`section_id`,`company_id`)
          SELECT parent,ledger_id,dealer_name_e,section_id,company_id FROM data_upload_helper
          WHERE 1";
    ...
    if(isset($_POST["Import"])){
            ...
            if($_FILES["file"]["size"] > 0)
            {
                ...
                {   //It will insert a row to our subject table from our csv file`
						//if(isset($emapData[0])) {
                            $sql = "INSERT INTO `data_upload_helper` (`dealer_custom_code`,`dealer_name_e`,`propritor_name_e`,`outlet_category`,`territory`,`town_code`,`area_code`,`region`,`section_id`,`company_id`,`status`,`source`,`source_id`,`mobile_no`,`address_e`,`dealer_code`,`ledger_id`)
            VALUES ('$emapData[0]','$emapData[1]','$emapData[3]','$emapData[2]','$emapData[4]','$emapData[5]','$emapData[6]','$emapData[7]','".$_SESSION['sectionid']."','".$_SESSION['companyid']."','UNMOVED','$source','$source_id','$emapData[8]','$emapData[9]','$emapData[10]','$emapData[11]')";
                        //}
...
{	unset($_POST);
    ...
    $results="Select code,dealer_code,dealer_custom_code,dealer_name_e,mobile_no,email as unit,rate as DP_price,ledger_id,parent  from data_upload_helper  where
