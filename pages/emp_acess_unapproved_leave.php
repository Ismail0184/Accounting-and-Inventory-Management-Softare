...
$current_status=find_a_field("".$table."","incharge_status","".$unique."=".$_GET[$unique]."");
...
//for modify..................................
    if(isset($_POST['confirm']))
    {
        ...
        $_POST['incharge_entry_at']=date("Y-m-d h:i:sa");
        $crud->update($unique);
        $type=1;
...
<form  name="addem" id="addem" class="form-horizontal form-label-left" method="post">
    ...
    <?php if($current_status!=$required_status && $current_status!="MANUAL" && $current_status!="RETURNED"){ echo '<h6 style="text-align:center; color:red; font-weight:bold"><i>This leave has been Approved!!</i></h6>';} else { ?>
        <table align="center" style="width:90%;font-size:12px;">

            <tr>
                <td>
                            <button type="submit" style="font-size:12px; float:left" onclick='return window.confirm("Are you confirm to Deleted?");' name="Deleted" id="Deleted" class="btn btn-danger">Cancel & Deleted</button>
                            <button type="submit" style="font-size:12px; float:right" onclick='return window.confirm("Are you confirm to Recommended the Requisition?");' name="confirm" id="confirm" class="btn btn-success">Check & Forward</button>
