...
$title="Pending Manpower Application";
$dfrom=date('Y-1-1');
$dto=date('Y-m-d');
...
$page="hrm_pending_manpower_requisition.php";
$crud      =new crud($table);
$$unique = $_GET[$unique];
        ...
        <div class="x_panel">
            ...
            <div class="x_content">
                <table class="table table-striped table-bordered" style="width:100%;font-size:12px">
                    ...
                    <tbody>
                    <? 	$res=mysql_query('select r.'.$unique.',r.'.$unique.' as Req_No,r.'.$unique_field.',r.authorized_at,
				 (SELECT concat(p2.PBI_NAME," # ","(",de.DESG_SHORT_NAME,")") FROM 
							...
							designation de 
							 ...
							 p2.PBI_DEPARTMENT=d.DEPT_ID) as Req_By,r.no_of_vacancies,r.Priority,(select PBI_NAME from personnel_basic_info where PBI_ID=r.authorise_by) as authorise_by
				  from '.$table.' r
				  WHERE 				  
				  hrm_viewed in ("","NO")				  	  
				   order by r.'.$unique.' DESC');
                    while($req=mysql_fetch_object($res)){
                        ...
                        <tr style="cursor: pointer" onclick="DoNavPOPUP('<?=$req->$unique;?>', 'TEST!?', 600, 700)">
                            ...
                            <td><?=$req->Priority;?></td>
                            <td><?=$req->authorise_by;?></td>
                            <td><?php if($req->authorized_at>0) echo $req->authorized_at; else echo "<font style='text-align: center'>Pending</font>"?></td>
                        </tr>
                    <?php } ?>
            ...
            <div class="x_content">
                ...
                <form  name="addem" id="addem" class="form-horizontal form-label-left" method="post">
                    ...
                    <table class="table table-striped table-bordered" style="width:100%;font-size:12px">
                        ...
                        <tr>
                            <th>Authorized By :</th>
                            <td><?=find_a_field("personnel_basic_info","PBI_NAME","PBI_ID=".$datas->authorise_by."");?></td>
                            <th>Authorized At : </th>
                            <td><?=$datas->recommend_at;?></td>
                        </tr>
                    ...
                    if(isset($_POST[Granted])){
                        mysql_query("Update ".$table." SET hrm_viewed='$authorused_status',hrm_viewed_date='$todayss' where ".$unique."=".$_GET[$unique]."");
                         echo "<script>self.opener.location = '$page'; self.blur(); </script>";
                        echo "<script>window.close(); </script>";
                    }  ?>

    <?php if($current_status_HR=="YES"){ echo '<h6 style="text-align:center; color:red; font-weight:bold"><i>This Manpower Application has been GRANTED !!</i></h6>';} else { ?>
        <?php if($current_status!=$required_status){ echo '<h6 style="text-align:center; color:red; font-weight:bold"><i>This Manpower Application has not yet been approved. Please wait until approval !!</i></h6>';} else { ?>
            <table style="width:100%;font-size:12px">
                            ...
                            <tr>
                                ...
                                <td><div class="form-group">
                                        <div class="col-md-6 col-sm-6 col-xs-12">
                                            <button type="submit" onclick='return window.confirm("Are you confirm to Granted the Application?");' name="Granted" id="Granted" class="btn btn-success">Granted & Take Action</button>
                                        </div></div></td></tr></table>
                    <?php }} ?>
