...
$page="warehouse_payment_voucher.php";
    ...
    if (isset($_POST['add'])) {
        ...
        if ((($_POST[dr_amt] || $_POST[cr_amt]) > 0) && ($_SESSION[initiate_debit_note]>0)) {
            add_to_payment($_SESSION[initiate_debit_note],$date, $proj_id, $_POST[narration], $_POST[ledger_id], $_POST[dr_amt],
                $_POST[cr_amt], $type,$_SESSION['warehouse'],'Warehouse',$_POST[Cheque_No],$c_date,$_POST[Cheque_of_bank],$manual_payment_no,$_POST[cc_code],$_POST[subledger_id],MANUAL,$ip,$_POST[receipt_date],$_SESSION[sectionid],$_SESSION[companyid],$_SESSION[userid],$create_date,$now,$day,$thisday,$thismonth,$thisyear,$receive_ledger);
				if ($_POST[rcved_remining]==$_POST[dr_amt]) {
                    add_to_payment($_SESSION[initiate_debit_note], $date, $proj_id, $_POST[narration], $_POST[cash_bank_ledger], 0,
                        $_POST[amount], Credit, $_SESSION['warehouse'], 'Warehouse', $_POST[Cheque_No], $c_date, $_POST[Cheque_of_bank], $manual_payment_no, $_POST[cc_code], $_POST[subledger_id], MANUAL, $ip, $_POST[receipt_date], $_SESSION[sectionid], $_SESSION[companyid], $_SESSION[userid], $create_date, $now, $day, $thisday, $thismonth, $thisyear, $_POST[ledger_id]);
                }
...
// data query..................................
 $condition=$unique."=".$_SESSION['initiate_debit_note'];
    ...
    { $$key=$value;}
	$inputted_amount=find_a_field(''.$table_payment.'','SUM(dr_amt)',''.$payment_unique.'="'.$_SESSION['initiate_debit_note'].'"');
	}
...
<?php require_once 'body_content_entry_mod.php'; ?>
                    ...
                    <div class="col-md-8 col-xs-12">
                        ...
                        <tr>
                         ...
                         <tr>
                            <th style="">Cash or Bank Account</th><th>:</th>
                            <td colspan="3" style="padding-top: 5px;">
                            <select class="select2_single form-control" style="width:98%; font-size: 11px" tabindex="-1" required="required"  name="cash_bank_ledger" id="cash_bank_ledger">
                                    <option></option>
                                    <? $sql = "SELECT distinct wed.value,concat(wed.value,' : ', al.ledger_name)  FROM                        
                        warehouse_essential_data wed, accounts_ledger al where wed.parameter_name in ('cash_ledger','bank_ledger') and warehouse_id='".$_SESSION['warehouse']."' and al.ledger_id=wed.value and wed.status>0 ";
                                advance_foreign_relation($sql,$cash_bank_ledger);?>
                                </select></td>
                            <td ><input type="number" id="amount"   value="<?=$amount;?>" name="amount"  class="form-control col-md-7 col-xs-12" placeholder="Paid Amt" required="required" style="width: 90%; margin-top: 5px; height: 38px; font-size: 11px; vertical-align: middle" step="any" min="1" />
                            </td>
                        </tr>                    
                    </table>
...
<?php if($_SESSION[initiate_debit_note]):  ?>
                                        ...
                                        <tr>
                                            <td style="width: 25%; vertical-align: middle" align="center">
                                                <select class="select2_single form-control" style="width:100%; font-size: 11px" tabindex="-1" required="required"  name="ledger_id">
                                                    <option></option>
                                                    <? $sql = "SELECT distinct wed.value,concat(wed.value,' : ', al.ledger_name)  FROM                        
                        warehouse_essential_data wed, accounts_ledger al where wed.parameter_name='expenses_ledger' and warehouse_id='".$_SESSION['warehouse']."' and al.ledger_id=wed.value and wed.status>0 ";
                                advance_foreign_relation($sql,$edit_value->ledger_id);?>
                                            ...
                                            <td align="center" style="width: 10%;vertical-align: middle">
                                                <select class="select2_single form-control" style="width:100%" tabindex="-1" required   name="cc_code" id="cc_code">
                                                    <option></option>
                                                    <? $sql_cc_code = "SELECT distinct wed.value,concat(wed.value,' : ', cc.center_name)  FROM                        
                        warehouse_essential_data wed, cost_center cc where wed.parameter_name='cc_code' and warehouse_id='".$_SESSION['warehouse']."' and cc.id=wed.value and wed.status>0 ";
                                advance_foreign_relation($sql_cc_code,$edit_value->ledger_id);?>
                                                </select></td>
