...
if(prevent_multi_submit()){
	 if(isset($_POST['confirm'])){ 
			...
			$_POST[grn_inventory_type]='Asset';	
...
//////////////////////////////////////////////////////////			
			...
			$total_amount=$total_amount+$_POST['total_amt'];
			}
...
////////////////////// ledger GET//////////////
    ...
    elseif($tax_amtcr>0){
		...
		if($purchaseamt>0){
		insert_into_secondary_journal($_POST['rec_date'], $proj_id, $jv, $date, $purchase_ledger, $nerration, $purchaseamt, 0, Purchase, $_POST[pr_no], $$unique, 0, 0, $_SESSION[usergroup], $c_no, $c_date, $create_date, $ip, $now, $day, $thisday, $thismonth, $thisyear,Asset);
		if($tax_ait>0){
		$nerration_tax=$nerration.', Tax # '.$get_tax_ait.' %';	
		insert_into_secondary_journal($_POST['rec_date'], $proj_id, $jv, $date, $tax_ait_ledger, $nerration_tax, $tax_ait, 0, Purchase, $_POST[pr_no], $$unique, 0, 0, $_SESSION[usergroup], $c_no, $c_date, $create_date, $ip, $now, $day, $thisday, $thismonth, $thisyear,Asset);			
		}
		if($tax_amt>0){
		$nerration_VAT=$nerration.', VAT # '.$_POST[tax].'%, VAT Date # '.$_POST[VAT_challan_Date];
		insert_into_secondary_journal($_POST['rec_date'], $proj_id, $jv, $date, $tax_ledger, $nerration_VAT, $tax_amt, 0, Purchase, $_POST[pr_no], $$unique, 0, 0, $_SESSION[usergroup], $c_no, $c_date, $create_date, $ip, $now, $day, $thisday, $thismonth, $thisyear,Asset);			
		}
		if($tax_amtcr>0){
			$nerration_VAT=$nerration.', VAT # '.$_POST[tax].'%, VAT Date # '.$_POSR[VAT_challan_Date];
		 insert_into_secondary_journal($_POST['rec_date'], $proj_id, $jv, $date, $tax_ledgercr, $nerration_VAT, 0, $tax_amtcr, Purchase, $_POST[pr_no], $$unique, 0, 0, $_SESSION[usergroup], $c_no, $c_date, $create_date, $ip, $now, $day, $thisday, $thismonth, $thisyear,Asset);	
		}
        insert_into_secondary_journal($_POST['rec_date'], $proj_id, $jv, $date, $vendor_ledger, $nerration, 0, $vendoramount, Purchase, $$unique, $$unique, 0, 0, $_SESSION[usergroup], $c_no, $c_date, $create_date, $ip, $now, $day, $thisday, $thismonth, $thisyear,Asset);
		}
...
{
	var val=form.rec_no.options[form.rec_no.options.selectedIndex].value;
	self.location='<?=$page;?>?po_no=<?=$_GET[po_no]?>&MAN_ID=' + val ;
}
...
<table width="98%"  border="0" align="center"  style="font-size: 11px; background-color: blanchedalmond; padding:20px">
 <tr>
    ...
    <th>Rcv. Date</th>
    <th style="width:1%"> : </th>
    <td><input style="width:90%" name="rec_date" MAX="<?=date('Y-m-d')?>" type="date"  value="<?=$ch_date;?>"  required="required" /></td>
    
    <th>VAT Date</th>
    <th style="width:1%"> : </th>
    <td><input style="width:90%"  name="VAT_challan_Date" MAX="<?=date('Y-m-d')?>" type="date"  value="<?=$VAT_challan_Date?>" /></td>
    
    <th>Challan Date</th>
    <th style="width:1%"> : </th>
    <td><input style="width:90%" name="ch_date" type="date" MAX="<?=date('Y-m-d')?>"  value="<?=$ch_date;?>" required="required" /></td>
    </tr>
    </table>
    ...
    <td><input style="width:90%" name="tax" type="text"  value="<? if($tax>0) echo $tax;?>" title="VAT" placeholder=" VAT (%)" required="required" /></td>    <td><input style="width:90%" name="ch_no" type="text" title="Chalan No" placeholder="Chalan No"  value="<?=$ch_no;?>" required="required" /></td>    
    ...
    <td><select style=" height:23px; width:90%"  name="qc_by" type="text"  id="qc_by" required="required">
        ...
        <option value="Afnan Azad ,SR. Executive, Procurement">Afnan Azad</option>
          ...
          <tr>
           ...
           <input type="hidden" style="width:70px" name="lot_number<?=$row->id?>" id="lot_number<?=$row->id?>" value="<?=$_SESSION['POunique_id']++;?>" readonly />
            <td><?=$row->item_id?>
			  ...
			  <? if($unrec_qty>0){$cow++;?>
                <input name="chalan_<?=$row->id?>" onkeyup="doAlert(this.form);" type="text" id="chalan_<?=$row->id?>" style="width:70px; float:none" value="<?=$MAN_details->qty;?>"  />
                <? } else echo 'Done';?></td>
...
<td align="center"> <button type="submit" name="confirm" style="font-size:12px; float:right" class="btn btn-primary">Received Assets</button>
</td>
<? } else {echo '<h6 style="color:red;"><i>This purchase order has not yet been approved. Wait until approval !!</i></h6>';} ?>
