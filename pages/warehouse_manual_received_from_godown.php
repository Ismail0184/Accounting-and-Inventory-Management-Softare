...
$title='Goods Received (IGR)';

$now=time();
$unique='pi_no';
$table="production_issue_master";
$table_details="production_issue_detail";
$page="warehouse_manual_received_from_godown.php";
$crud      =new crud($table);
...
{   $up="UPDATE ".$table." SET verifi_status='CHECKED' where ".$unique."='$_SESSION[pi_tr]'";
    $update_table_master=mysqli_query($conn, $up);
    $up2="UPDATE ".$table_details." SET verifi_status='CHECKED',status='CHECKED' where ".$unique."='".$_SESSION['pi_tr'];
            ...
            <form action="<?=$page?>" class="form-horizontal form-label-left" method="post" style="font-size: 11px">
                <table style="width:100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="form-control" style="width:90%; font-size: 11px" tabindex="-1" required="required"  name="warehouse_from">
                                <option></option>
                        ...
                        <th style="text-align:center">:</th>
                        <td>
                            <select class="form-control" style="width:90%; font-size: 11px" tabindex="-1" required="required"  name="warehouse_to">
                                <?php if(isset($_SESSION['initiate_production_transfer'])>0): ?>
                                    <option value="<?=$warehouse_to?>" selected><?=$warehouse_to?> : <?=find_a_field('warehouse','warehouse_name','warehouse_id='.$warehouse_to)?></option>
                                <?php else: ?>
                                    <?=foreign_relation("warehouse","warehouse_id","concat(warehouse_id,' : ',warehouse_name)",$warehouse_to, "warehouse_id in ('".$_SESSION['warehouse']."')");?>
                                <?php endif; ?>
                            </select>
                        </td>
