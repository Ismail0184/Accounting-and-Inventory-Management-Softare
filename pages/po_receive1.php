...
<table width="98%"  border="0" align="center"  style="font-size: 11px; background-color: blanchedalmond; padding:20px">
 <tr>
    ...
    <td><input style="width:90%;"  name="VAT_challan" type="text" id="VAT_challan" value="<?=find_a_field('MAN_details','VAT_challan','status="VERIFIED" and MAN_ID="'.$_GET[MAN_ID].'" and po_no='.$_GET[po_no]); ?>" title="VAT Challan No" placeholder="VAT Challan No" required="required"/></td>
          ...
          <tr>
           ...
           <input type="hidden" style="width:70px" name="lot_number<?=$row->id?>" id="lot_number<?=$row->id?>" value="<?=$_SESSION['POunique_id']++;?>" readonly />
            <td><?=$row->item_id?>
			  ...
			  <? if($unrec_qty>0){$cow++;?>
                <input name="chalan_<?=$row->id?>" onkeyup="doAlert<?=$row->id?>(this.form);" type="text" id="chalan_<?=$row->id?>" style="width:70px; float:none" value="<?=$MAN_details->qty;?>"  />
                <? } else echo 'Done';?></td>
                <SCRIPT language=JavaScript>
