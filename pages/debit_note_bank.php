...
//for single FG Add...........................
    if (isset($_POST['add'])) {
        ...
        if (($_POST[dr_amt] && $_POST[cr_amt]) > 0) {
            ...
            if ((($_POST[dr_amt] || $_POST[cr_amt]) > 0) && ($_SESSION[initiate_bank_debit_note]>0)) {
            ...
            if ($_FILES["attachment"]["tmp_name"] != '') {
                $path = '../page/receipt_attch/' . $_SESSION[initiate_bank_debit_note] . '.jpg';
...
where 
 ...
 ";
        ...
        $up_payment="UPDATE ".$table_payment." SET entry_status='UNCHECKED' where ".$payment_unique."=".$_SESSION['initiate_bank_debit_note']."";
...
if (isset($_POST['cancel'])) {
    ...
    $condition =$payment_unique."=".$_SESSION['initiate_bank_debit_note'];
    $crud->delete_all($condition);
    $crud = new crud($table_journal_master);
    $condition=$unique."=".$_SESSION['initiate_bank_debit_note'];
    $crud->delete($condition);
    unset($_SESSION['initiate_bank_debit_note']);
    unset($_POST);
    unset($$unique);
}
$COUNT_details_data=find_a_field(''.$table_payment.'','Count(id)',''.$payment_unique.'='.$_SESSION['initiate_bank_debit_note'].'');

// data query..................................
if(isset($_SESSION['initiate_bank_debit_note']))
{   $condition=$unique."=".$_SESSION['initiate_bank_debit_note'];
...
from  secondary_journal_bank a where a.tr_from='Payment' and a.user_id='$_SESSION[userid]' and  a.section_id='$_SESSION[sectionid]' and a.company_id='$_SESSION[companyid]'  group by a.tr_no  order by a.id desc limit 10"; 
$rs="Select 
j.id as jid,
            ...
            <form action="<?=$page;?>" enctype="multipart/form-data" method="post" name="addem" id="addem" style="font-size: 11px" >
                <table align="center" style="width:100%">
                        ...
                        <td style="width:50%; vertical-align: middle">
                            <div class="form-group" style="width: 100%">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="date" id="voucher_date"  required="required" name="voucher_date" value="<?=($voucher_date!='')? $voucher_date : date('Y-m-d') ?>" max="<?=date('Y-m-d');?>" min="<?=date('Y-m-d', strtotime($date .' -'.find_a_field('acc_voucher_config','back_date_limit','1'). 'day'));?>" class="form-control col-md-7 col-xs-12" style="width: 130px;font-size: 11px;" >
                                </div>
                            </div></td>


                        <td style="width:50%; vertical-align: middle"><div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="text" required="required" name="<?=$unique?>" id="<?=$unique?>"  value="<?=($_SESSION['initiate_bank_debit_note']!='')? $_SESSION['initiate_bank_debit_note'] : automatic_voucher_number_generate($table_payment,$payment_unique,1,5);?>" class="form-control col-md-7 col-xs-12"  readonly style="width: 130px; font-size: 11px;">
                                </div>
                            </div>
                        </td></tr>
                    ...
                    <tr>
                        ...
                        <td><div class="form-group">
                                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name" style="width: 40%">Paid to:</label>
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="text" id="paid_to"  value="<?=$paid_to;?>" name="paid_to" class="form-control col-md-7 col-xs-12" style="width: 130px; margin-top: 5px; font-size: 11px;" >
                                </div></div></td>
                    ...
                    <tr> <td><div class="form-group">
                        ...
                        <td><div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="date" id="Cheque_Date"   value="<?=$Cheque_Date;?>" name="Cheque_Date"  class="form-control col-md-7 col-xs-12"  style="width: 130px; margin-top: 5px; font-size: 11px;">
                                </div></div></td></tr>
                ...
                <?php if($_SESSION[initiate_bank_debit_note]){
                    ...
                    <div class="form-group" style="margin-left:<?=$ml;?>%; margin-top: 15px">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <button type="submit" name="modify" class="btn btn-primary" onclick='return window.confirm("Are you confirm to Update?");' style="font-size: 11px">Update Cheque Payment Voucher</button>
                        </div></div>

                    <div class="form-group" <?=$display;?>>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <a  href="voucher_print_preview.php?v_type=payment&vo_no=<?=$_SESSION[initiate_bank_debit_note];?>&v_date=<?=$voucher_date;?>" target="_blank" style="color: blue; text-decoration: underline; font-size: 11px; font-weight: bold; vertical-align: middle">View Cheque Payment Voucher</a>
                        </div></div>
                <?php   } else {?>
                    <div class="form-group" style="margin-left:40%; margin-top: 15px">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <button type="submit" name="initiate" class="btn btn-primary" style="font-size: 12px">Initiate Bank Voucher</button>
                        </div></div>
                <?php } ?>
...
<?=recentvoucherview($sql2,'voucher_print1.php','payment_bank','207px');?>
<?php if($_SESSION[initiate_bank_debit_note]):  ?>
    <form action="<?=$page;?>" enctype="multipart/form-data" name="addem" id="addem" style="font-size: 11px" class="form-horizontal form-label-left" method="post">
        <input type="hidden" name="payment_no" id="payment_no" value="<?=$_SESSION[initiate_bank_debit_note];?>">
        ...
        <table align="center" class="table table-striped table-bordered" style="width:98%; font-size: 11px">
            ...
            <tr>
                ...
                <td style="width:15%;vertical-align: middle" align="center">
                    <textarea  id="narration" style="width:100%; height:37px; font-size: 11px; text-align:center"  name="narration" value="<?=$_POST[narration];?>"  class="form-control col-md-7 col-xs-12" autocomplete="off" ><?=($edit_value->narration!='')? $edit_value->narration : $_POST['narration'];?></textarea></td>
                ...
                <td align="center" style="width:10%; vertical-align: middle">
                    ...
                    <?php } else { ?>
                        ...
                        <input type="number" id="cr_amt" style="width:98%; height:25px; font-size: 11px; text-align:center; margin-top: 5px" name="cr_amt" placeholder="Credit" class="form-control col-md-7 col-xs-12" autocomplete="off" step="any" min="1" /><?php } ?></td>
