...
// data query..................................
if(isset($_SESSION['initiate_journal_note']))
{   $condition=$unique."=".$_SESSION['initiate_journal_note'];
...
<script type="text/javascript">
    ...
    {myWindow = window.open("voucher_view_popup_ismail.php?v_type=journal_info&view=Show&in=journal_info&v_no="+lk, "myWindow", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no,directories=0,toolbar=0,scrollbars=1,location=0,statusbar=1,menubar=0,resizable=1,width=800,height=500,left = 250,top = -1");}
</script>
            ...
            <form action="" enctype="multipart/form-data" method="post" name="addem" id="addem" style="font-size: 11px" >
                <table align="center" style="width:100%">
                    <tr>
                        <td style="width:50%;">
                            <div class="form-group" style="width: 100%">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="date" id="voucher_date"  required="required" name="voucher_date" value="<?=$voucher_date;?>" max="<?=$_SESSION[create_date];?>" class="form-control col-md-7 col-xs-12" style="width: 130px; font-size: 11px;" >
                                </div>
                            </div></td>
                        ...
                        <td style="width:50%"><div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input type="text" required="required" name="<?=$unique?>" id="<?=$unique?>"  value="<?php if($_SESSION['initiate_journal_note']>0){ echo $_SESSION['initiate_journal_note'];} else { echo
                                    $_SESSION['JVNONEW']; } ?>" class="form-control col-md-7 col-xs-12"  readonly style="width: 130px; font-size: 11px;">
                                </div>
                            </div>
                        </td></tr>
                ...
                <?php if($_SESSION[initiate_journal_note]){
                    ...
                    <div class="form-group" style="margin-left:<?=$ml;?>%; margin-top: 15px">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <button type="submit" name="modify" class="btn btn-primary" onclick='return window.confirm("Are you confirm to Update?");' style="font-size: 12px">Update Journal Voucher</button>
                        </div></div>

                    <div class="form-group" <?=$display;?>>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <a  href="voucher_print_preview.php?v_type=journal_info&vo_no=<?=$_SESSION[initiate_journal_note];?>&v_date=<?=$voucher_date;?>" target="_blank" style="color: blue; text-decoration: underline; font-size: 12px; font-weight: bold; vertical-align: middle">View Journal Voucher</a>
                        </div></div>
                <?php   } else {?>
                    <div class="form-group" style="margin-left:40%; margin-top: 15px">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <button type="submit" name="initiate" class="btn btn-primary" style="font-size: 12px">Initiate Journal Voucher</button>
                        </div></div>
                <?php } ?>
...
<div class="col-md-4 col-xs-12">
    <div class="x_panel">
        ...
        <div class="x_content" style="overflow:scroll; height:210px;">
            <? $res="select a.jv_no,@s:=@s+1 as SL, a.tr_no as Voucher_No,a.jvdate as Date,SUM(a.dr_amt) as amount
from  journal a,(SELECT @s:= 0) AS s where a.tr_from='journal_info' and a.user_id='$_SESSION[userid]' and a.section_id='$_SESSION[sectionid]' and a.company_id='$_SESSION[companyid]'  group by a.tr_no  order by a.id desc limit 10";
            ...
            <tr>
                <td style="width: 25%; vertical-align: middle" align="center">
                    <select class="select2_single form-control" style="width:100%; font-size: 11px" tabindex="-1" required="required"  name="ledger_id">
                        <option></option>
                        <?php foreign_relation('accounts_ledger', 'ledger_id', 'CONCAT(ledger_id," : ", ledger_name)', $ledger_id, '1'); ?>
                    </select></td>
...
journal_info j,
 ...
 j.journal_info_no='".$_SESSION['initiate_journal_note']."'
 ";
            echo $crud->link_report_voucher($rs,$link);?>
            ...
            <table align="center" class="table table-striped table-bordered" style="width:98%">
                ...
                while($uncheckrow=mysqli_fetch_array($re_query)){
                    ...
                    {   $res=mysqli_query($conn, ("DELETE FROM ".$table_journal_info." WHERE id='$ids'")); ?>
                    ...
                    <tr>
                        ...
                        <td style="vertical-align:middle; text-align: center"><?=$uncheckrow[cname]; ?></td>
                        <td style="vertical-align:middle; width: 20%"><?=$uncheckrow[narration] ;?></td>
                        <td style="vertical-align:middle; text-align: center; width: 10%"><a href="#" target="_blank" style="color: blue; text-decoration: underline"><?=$uncheckrow[payment_no] ;?></a></td>
        ...
        <?php if($COUNT_details_data>0) { ?>
            <?php if($totaldr==$totalcr){ ?>
                <button style="float: right; font-size: 12px; margin-right: 1%" type="submit" name="confirmsave" class="btn btn-success" onclick='return window.confirm("Are you confirm?");'>Confirm and Finish Voucher </button>
            <?php } else { ?>
                <font  style="font-size: 13px; color: red; font-weight: bold; float: right; margin-right: 1%">Invalid Journal Voucher. Debit and Credit Amount Are Not Equal !!</font>
            <?php }} ?>
