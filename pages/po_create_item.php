...
$res='select a.id, concat(b.item_id," # ", b.item_name," # ",a.item_details) as item_description,a.unit_name as unit,a.rate as unit_price,a.qty,a.amount from purchase_invoice a,item_info b where b.item_id=a.item_id and a.po_no='.$_SESSION[initiate_po_no];
...
if (isset($_POST['cancel'])) {
        ...
        $condition = $unique . "=" . $_SESSION[initiate_po_no];
        $crud->delete_all($condition);
		$qoutationDoc_delete = '../page/po_documents/qoutationDoc/'.$_SESSION[initiate_po_no].'.pdf';
        unlink($qoutationDoc_delete);
        $mailCommDoc_delete = '../page/po_documents/mailCommDoc/'.$_SESSION[initiate_po_no].'.pdf';
...
$COUNT_details_data=find_a_field(''.$table_details.'','Count(id)',''.$unique.'='.$_SESSION[initiate_po_no].'');
endif;

$vendor=find_all_field('vendor','','vendor_id='.$_SESSION[select_vendor_PO].'');
    ...
    <table align="center" style="width: 60%;; font-size: 11px">
        <tr><td>Vendor / Party</td>
            ...
            <td style="vertical-align: middle"><select class="select2_single form-control" style="width:300px; font-size: 11px" tabindex="-1" required="required"  name="vendor_id" id="vendor_id">
                    <option></option>
                    <?php if(isset($_SESSION[select_vendor_PO])>0): ?>
                        <?php foreign_relation('vendor', 'vendor_id', 'CONCAT(vendor_id," : ", vendor_name)', $_SESSION[select_vendor_PO], 'vendor_id='.$_SESSION[select_vendor_PO]); ?>
                    <?php else: ?>
                        <? foreign_relation('vendor','vendor_id','concat(vendor_id," : ",vendor_name)',$_SESSION[select_vendor_PO], 'status="ACTIVE"');?>
                    <?php endif; ?>
                </select>
            </td>
...
<div class="col-md-12 col-sm-12 col-xs-12">
  ...
  <form action="<?=$page;?>" enctype="multipart/form-data" class="form-horizontal form-label-left" method="post" name="codz" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
      ...
      <th style="width: 10%">WO No</th>
         ...
         <td style="width: 20%">
                    <input type="text" id="po_no" style="width:20%" name="po_no" value="<?=$_SESSION[initiate_po_no];?>" readonly class="form-control col-md-7 col-xs-12" >
                    ...
                    <input type="hidden" id="po_id"   required="required" name="po_id" value="<?=($_SESSION[1]!='')? $_SESSION[1] : automatic_number_generate("","purchase_master","po_id","create_date='".date('Y-m-d')."'"); ?>" class="form-control col-md-7 col-xs-12"  readonly >
         </td>
          ...
          <th style="width: 10%">Vendor</th>
              ...
              <select style="width: 80%;margin-top: 2px;" class="select2_single form-control"  required name="vendor_id" id="vendor_id">
                              <option></option>
                              <? foreign_relation('vendor','vendor_id','concat(vendor_id," : ",vendor_name)',$_SESSION[select_vendor_PO],'vendor_id='.$_SESSION[select_vendor_PO]);?>
                          </select>
                          </td>
          ...
          <th>Quotation Date</th>
            ...
            <th>Commission (%)</th>
              <th style="width:1%; text-align:center">:</th>
              <td>
                  <input type="number" readonly name="commission" value="<?=($_SESSION['initiate_po_no']>0)? $commission : $vendor->commission ?>" style="width: 80%;margin-top: 2px; font-size:11px" class="form-control col-md-7 col-xs-12" >
              </td>
