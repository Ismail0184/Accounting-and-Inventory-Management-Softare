    ...
    if (isset($_POST['add'])) {
        ...
        if ((($_POST[dr_amt] || $_POST[cr_amt]) > 0) && ($_SESSION[initiate_bank_debit_note]>0)) {
            add_to_bank_payment($_SESSION[initiate_bank_debit_note],$date, $proj_id, $_POST[narration], $_POST[ledger_id], $_POST[dr_amt],
                0, $type,$cur_bal,$_POST[paid_to],$_POST[cheque_id],$_POST[maturity_date],$_POST[Cheque_of_bank],$manual_payment_no,$_POST[cc_code],$_POST[cash_bank_ledger],MANUAL,$ip,$_POST[receipt_date],$_SESSION[sectionid],$_SESSION[companyid],$_SESSION[userid],$create_date,$now,$day,$thisday,$thismonth,$thisyear,$receive_ledger);
            ...
            if ($_POST[rcved_remining]==$_POST[dr_amt]) {
                add_to_bank_payment($_SESSION[initiate_bank_debit_note],$date, $proj_id, $_POST[narration], $_POST[cash_bank_ledger], 0,
                    $_POST[amount], $type,$cur_bal,$_POST[paid_to],$_POST[cheque_id],$_POST[maturity_date],$_POST[Cheque_of_bank],$manual_payment_no,$_POST[cc_code],$_POST[subledger_id],MANUAL,$ip,$_POST[receipt_date],$_SESSION[sectionid],$_SESSION[companyid],$_SESSION[userid],$create_date,$now,$day,$thisday,$thismonth,$thisyear,$receive_ledger);
             }
        }
...
if($_SESSION['initiate_bank_debit_note']>0){
$rs = "Select 
j.id as jid,
j.payment_no,
j.payment_date,
...
$edit_value=find_all_field(''.$table_payment.'','','id='.$_GET[id].'');
	...
	if (isset($_POST['confirmsave'])) {			
       $up_payment="UPDATE ".$table_payment." SET entry_status='PREMATURE' where ".$payment_unique."=".$_SESSION['initiate_bank_debit_note']."";
...
// data query..................................
 $condition=$unique."=".$_SESSION['initiate_bank_debit_note'];
    ...
    { $$key=$value;}
	$inputted_amount=find_a_field(''.$table_payment.'','SUM(dr_amt)',''.$payment_unique.'="'.$_SESSION['initiate_bank_debit_note'].'"');
	}
...
<?php require_once 'body_content_entry_mod.php'; ?>
                    ...
                    <div class="col-md-8 col-xs-12">
                        ...
                        <tr>
                            ...
                            <td><input type="text" required="required" name="<?=$unique?>" id="<?=$unique?>"  value="<?=($_SESSION['initiate_bank_debit_note']!='')? $_SESSION['initiate_bank_debit_note'] : automatic_voucher_number_generate($table_payment,$payment_unique,1,5);?>" class="form-control col-md-7 col-xs-12" readonly style="width: 90%; font-size: 11px;"></td>
                        </tr>
...
<?php if($_SESSION[initiate_bank_debit_note]):  ?>
                                ...
                                <form action="<?=$page;?>" enctype="multipart/form-data" name="addem" id="addem" style="font-size: 11px" class="form-horizontal form-label-left" method="post">
                                  <input type="hidden" name="<?=$unique?>" id="<?=$unique?>" value="<?=$_SESSION[initiate_bank_debit_note];?>">
                                        ...
                                        <tr>
                                            <td style="width: 25%; vertical-align: middle" align="center">
                                                <select class="select2_single form-control" style="width:100%; font-size: 11px" tabindex="-1" required="required"  name="ledger_id">
                                                    <option></option>
                                                <?php foreign_relation('accounts_ledger', 'ledger_id', 'CONCAT(ledger_id," : ", ledger_name)', $edit_value->ledger_id, 'ledger_group_id in ("2002") and status=1'); ?>
                                                </select></td>
                                            ...
                                            <td align="center" style="width:10%; vertical-align: middle">
                                                 <select class="select2_single form-control" style="width:100%" tabindex="-1"   name="cheque_id" id="cheque_id">
                                                     <option></option>
                                                     <?php foreign_relation('Cheque_Book', 'id', 'CONCAT(id,"-", Cheque_number)', $edit_value->cheque_id, 'status="UNUSED" and bank_id='.$cash_bank_ledger.''); ?>
                                                 </select>
                                                </td>
...
<?=voucher_delete_edit($rs,$unique,$_SESSION['initiate_bank_debit_note'],$COUNT_details_data);?><br><br>
<?php endif;?>
