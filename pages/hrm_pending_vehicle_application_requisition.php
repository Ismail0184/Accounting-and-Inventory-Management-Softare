...
$title="Pending Vehicle Application List";
$dfrom=date('Y-1-1');
$dto=date('Y-m-d');
...
$page="hrm_pending_vehicle_application_requisition.php";
$crud      =new crud($table);
$$unique = $_GET[$unique];
             ...
             <table class="table table-striped table-bordered" style="width:100%;font-size:11px">
				 ...
				 (SELECT concat(p2.PBI_NAME," # ","(",de.DESG_SHORT_NAME,")") FROM 
							...
							designation de 
							 ...
							 p2.PBI_DEPARTMENT=d.DEPT_ID) as Req_By,r.travel_purpose,r.Priority,(select PBI_NAME from personnel_basic_info where PBI_ID=r.authorised_person) as approved_by
				  from '.$table.' r
				  WHERE 
                 ...
                 <? 	$res=mysql_query('Select td.*  from '.$table_details.' td
				  ...
				  td.'.$unique.'='.$_GET[$unique].'');
                   ...
                   <tr>
								...
								mysql_query("Update ".$table." SET hrm_viewed='".$authorised_status."',hrm_viewed_date='$todayss' where ".$unique."=".$_GET[$unique]."");
					   		   echo "<script>self.opener.location = '$page'; self.blur(); </script>";
                               echo "<script>window.close(); </script>";
								}
								?>
                                     ...
                                     <tr><td> <div class="form-group">
                                          ...
                                          <td>                                          
                                           ...
                                           <button type="submit" onclick='return window.confirm("Are you confirm to Deleted?");' name="Deleted" id="Deleted" class="btn btn-danger">Cancel & Deleted</button>
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                           <button type="submit" onclick='return window.confirm("Are you confirm to GRANTED the Application?");' name="granted" id="granted" class="btn btn-success">GRANTED & SEND VEHICLE TO USER</button>
                                            </div></div></td></tr></table>           
                                            <?php }} ?>
