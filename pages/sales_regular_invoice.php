...
while($data=mysqli_fetch_object($query)){
  ...
  {  mysqli_query($conn, ("DELETE FROM ".$table_detail." WHERE id=".$data->id));
     mysqli_query($conn, ("DELETE FROM ".$table_detail." WHERE gift_on_order=".$data->id));
      unset($_POST);}
 ...
 <table align="center" style="width:98%; font-size: 11px" class="table table-striped table-bordered">
        ...
        <tbody>
                    ...
                    <? advance_foreign_relation(find_all_item($product_nature="'Salable','Both'"),($_GET[item_id]>0)? $_GET[item_id] : $edit_value->item_id);?>
                     ...
                     <td style="width:10%; vertical-align: middle" align="center">
                     <input type="number" id="unit_price" style="width:99%; height:37px; font-size:11px;text-align:center" min="1" step="any" required="required" value="<?=($_GET[item_id]>0)? $item_all->d_price : $edit_value->unit_price?>" readonly name="unit_price"  class="form-control col-md-7 col-xs-12" autocomplete="off" class="unit_price" ></td>
