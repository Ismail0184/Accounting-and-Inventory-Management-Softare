...
$sql2="select a.id,a.s_date as date,a.reason,a.leave_status from ".$table." a where a.PBI_ID=".$_SESSION[PBI_ID]." order by a.".$unique." desc limit 10"	;
?>
...
<?php require_once 'body_content.php'; ?>
         ...
         <select class="select2_single form-control" name="type" id="type" required style="width:91%" onchange="javascript:reload(this.form)">
              ...
              <th>Reason</th>
              <th style="width:1%; text-align:center">:</th><td style="width: 20%"><textarea  id="reason" style="width:100%;font-size:11px"  required   name="reason" value=""   class="form-control col-md-7 col-xs-12" ><?=$reason?></textarea></td>
              ...
              <th style="width:1%; text-align:center">:</th>
              <td><select class="select2_single form-control" style="width:203px;" tabindex="-1" required="required" name="leave_responsibility_name" id="leave_responsibility_name">
                                                <option></option>
                                                <?=advance_foreign_relation($sql_leave_responsibility_name,$leave_responsibility_name);?>
                                            </select></td>
               <th>Mobile</th>
              <th style="width:1%; text-align:center">:</th><td style="width: 20%"><input name="leave_mobile_number" type="text" id="leave_mobile_number" value="<?=$leave_mobile_number?>" style="font-size:11px; width:100%" placeholder="Mobile no. During Leave" class="form-control col-md-7 col-xs-12" required/></td>                             
                                            </tr>
              ...
              <tr>                    
              <th>Checke By</th>
              <th style="width:1%; text-align:center">:</th><td><select class="select2_single form-control" style="width:203px;" tabindex="-1" required="required" name="PBI_IN_CHARGE" id="PBI_IN_CHARGE">
                                                <option></option>
              ...
              <th style="width:1%; text-align:center">:</th>
              <td>
              <select class="select2_single form-control" style="width:250px;" tabindex="-1" required="required" name="PBI_DEPT_HEAD" id="PBI_DEPT_HEAD">
                                                <option></option>
          ...
          <th style="width:1%; text-align:center">:</th>
          <td colspan="4">
              <input type="date" id="s_date" style="width:35.5%; font-size:11px" required="required" name="s_date" value="<?=$s_date?>" onchange="cal()" class="form-control col-md-7 col-xs-12" >
...
!!</i></h6>';
                                            ...
                                            <?php }} else { echo "<h6 style='text-align:center; color:red; font-weight:bold'>First you need to select a type of leave.</h6>"; } ?> 
...
<?=recentdataview($sql2,'voucher_view_popup_ismail.php','hrm_leave_info','282px','Recent Leave Applications');?> 
<?=$html->footer_content();?>