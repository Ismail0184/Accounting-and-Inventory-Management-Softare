...
function page_view_permission($page,$page_id_GET)
{
$sql="select 
...
function add_to_journal_master($invoice,$transaction_date, $rfrom, $cno, $c_date, $bank, $status, $remarks, $voucyertype,$PBI_ID,$ip)
{
 ...
 $query_journal = mysql_query($journal_master);
}
...
PBI_ID='$PBI_ID' where voucherno='".$SESSIONVoucherno."' and section_id='$_SESSION[sectionid]' and company_id='$_SESSION[companyid]'";
 $query_journal = mysql_query($journal_master_update);
    ...
    ,$thisday,$thismonth,$thisyear,$receive_ledger)
{
 ...
 $recept="INSERT INTO `receipt` (
							...
							'$sectionid',
							'$companyid',
							'$userid','','$create_date','$now','$day','$thisday','$thismonth','$thisyear','$receive_ledger')";
    ...
    ,$thisday,$thismonth,$thisyear,$receive_ledger)
{
 ...
 $coutra="INSERT INTO `coutra` (
							...
							'$companyid',
							'$userid','$create_date','$now','$day','$thisday','$thismonth','$thisyear')";
 $query_receipt = mysql_query($coutra);
}
//// /receipt add data update
...
narration='$upnarration'  where id='".$ids."' and section_id='$_SESSION[sectionid]' and company_id='$_SESSION[companyid]'";
 $query_journal = mysql_query($Receiptupdate);
}
...
cr_amt='$upcr_amt',
narration='$upnarration'  where id='".$ids."' and section_id='$_SESSION[sectionid]' and company_id='$_SESSION[companyid]'";
 $query_journal = mysql_query($paymentupdate);
}
...
cr_amt='$upcr_amt',
narration='$upnarration'  where id='".$ids."' and section_id='$_SESSION[sectionid]' and company_id='$_SESSION[companyid]'";
 $query_journal = mysql_query($coutraupdate);
}
...
function ledger_create($ledger_id,$ledger_name,$ledger_group_id,$opening_balance,$balance_type,$depreciation_rate,$credit_limit, $opening_balance_on,$proj_id,$budget_enable='NO')
{
 ...
 $sql="INSERT INTO `accounts_ledger` 
		...
		`budget_enable`,
		group_for,section_id,company_id,create_date,entry_by)
		VALUES ('$ledger_id','$ledger_name', '$ledger_group', '$opening_balance', '$balance_type', '$depreciation_rate', '$credit_limit', '$opening_balance_on','$proj_id','$budget_enable','".$_SESSION['usergroup']."','".$_SESSION['sectionid']."','".$_SESSION['companyid']."','".$_SESSION['create_date']."','".$_SESSION['userid']."')";
 if(mysql_query($sql))
  return TRUE;
 else
  return FALSE;
...
function unauthorised_page_tried_to_view($page_id,$url,$time,$ip)
{
 $date=date('Y-m-d');
 $sql="INSERT INTO `unauthorised_page_tried_to_view` ( `page_id`, `url`, `user_id`, `date`, `time`, `ip`, `section_id`,`company_id`) VALUES ('$page_id', '$url', '$_SESSION[userid]', '$date','$time','$ip','$_SESSION[sectionid]','$_SESSION[companyid]')";
 mysql_query($sql);
 return 1;
}
?>
