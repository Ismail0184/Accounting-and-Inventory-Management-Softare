...
$title="Pending FG Purchase Requisition";
$dfrom=date('Y-1-1');
$dto=date('Y-m-d');
...
$page="hrm_pending_FG_purchase_requisition.php";
$crud      =new crud($table);
$$unique = $_GET[$unique];
				  ...
				  td.oi_no='.$_GET[$unique].'');
                        while($req_data=mysql_fetch_object($res)){
                            ...
                            <tr>
                                ...
                                <td style="text-align:center"><?=$taken=getSVALUE("".$table_details."", "SUM(qty)", " where oi_date between '$dfrom' and '$dto' and  oi_no='".$_GET[$unique]."' and item_id=".$req_data->item_id."")-$req_data->request_qty;?> <?=$req_data->unit_name;?>'s</td>
                            </tr>
                        <?php } ?>
                    ...
                    <?php
                    if(isset($_POST[update])){
                        mysql_query("Update ".$table." SET hrm_viewed='YES',hrm_viewed_by='".$_SESSION[PBI_ID]."',hrm_viewed_date='$todayss',status='COMPLETED' where oi_no=".$_GET[$unique]."");
                        echo "<script>self.opener.location = '$page'; self.blur(); </script>";
                        echo "<script>window.close(); </script>";
                    }
                    ?>
