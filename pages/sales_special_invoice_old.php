...
$in_stock_pcs = find_a_field('journal_item','sum(item_in)-sum(item_ex)','item_id="'.$_GET[item_id].'" and warehouse_id="'.$depot_id.'" ');					$ordered_qty = find_a_field('sale_do_details','sum(total_unit)','item_id="'.$_GET[item_id].'" and depot_id="'.$depot_id.'" and status in ("UNCHECKED","CHECKED","PROCESSING","MANUAL")');
...
$sql_item_id="SELECT i.item_id,concat(i.item_id,' : ',i.finish_goods_code,' : ',i.item_name,' (',sg.sub_group_name,')') FROM  item_info i,
							...
							item_group g WHERE  i.sub_group_id=sg.sub_group_id and 
							 ...
							 g.group_id and i.product_nature in ('Salable','Both')					 
							  order by i.item_name";

$res='select a.id,b.finish_goods_code as code,b.item_name,b.unit_name as unit,b.pack_size,a.pkt_unit as crt_qty,a.dist_unit as pcs,round(a.unit_price, 2) as unit_price,a.total_unit, a.total_amt
from 
...
$sql="select m.".$unique_master.",m.".$unique_master." as do_no,d.dealer_name_e as dealer,m.status
				  from ".$table_master." m,dealer_info d
				  WHERE m.entry_by=".$_SESSION['userid']." and
				  m.dealer_code=d.dealer_code 			  
				  order by m.".$unique_master." desc limit 10";
?>
        ...
        <input type="hidden" name="region" id="region" value="<?=$dealer->region;?>">
                    <table style="width:100%; font-size: 11px">
                        <tr>
                            ...
                            <th style="width: 15%">DO Date</th><th style="text-align: center; width: 2%">:</th><td><input type="date" style="width: 80%" name="do_date"  max="<?=date('Y-m-d');?>" value="<?=$do_date;?>"></td>
                            ...
                            <td><select style="width:80%; height:25px; font-size: 11px" name="do_type" id="do_type"  required>
                                        <option value=""></option>
                                        <option value="sales" <?php if($do_type=='sales') { echo 'selected';} else {};?>>Sales</option>
                        ...
                        <option></option>
                       <?=advance_foreign_relation($sql_plant,$depot_id);?>
                    </select></td>
                        </tr>
                        </table>
 ...
 <table align="center" style="width:98%; font-size: 11px" class="table table-striped table-bordered">
        ...
        <tbody>
                      ...
                      <td align="center">
                         ...
                         <td align="center" style="width:12%; background-color:#CCC">
                    <input type="text" id="inStock_ctn" style="width:45%; height:37px; font-weight:bold; text-align:center" value="<? $inStockCtn = ($in_stock_pcs-$ordered_qty)/$item_all->pack_size; $inStockCtn=(int)$inStockCtn; echo $inStockCtn;?>" placeholder="Ctn"   name="inStock_ctn" readonly class="form-control col-md-7 col-xs-12" autocomplete="off" >
                    
                    <input type="text" id="inStock_pcs" style="width:45%; height:37px; font-weight:bold; text-align:center" value="<?=($in_stock_pcs-$ordered_qty)-($inStockCtn*$item_all->pack_size)?>" placeholder="Pcs"   name="inStock_pcs" readonly class="form-control col-md-7 col-xs-12" autocomplete="off" >
...
<td style="width:12%" align="center">
                    ...
                    <input placeholder="Pcs" class="form-control col-md-7 col-xs-12" name="dist_unit" type="text"  id="dist_unit" style="width:45%; height:37px; margin-left:5px" onkeyup="avail_amount(),count()"/>
                    <input name="pkt_size" type="hidden" class="input3" id="pkt_size"  style="width:55px;"  value="<?=$item_all->pack_size?>" readonly/>
</td>

<td style="width:10%" align="center">
                        <input type="text" id="unit_price" style="width:99%; height:37px; font-size:11px;text-align:center"  required="required"  name="unit_price"  class="form-control col-md-7 col-xs-12" autocomplete="off" ></td>
                     ...
                     <script>
                                                $(function(){
                                                    $('#unit_price, #qty').keyup(function(){
                                                        ...
                                                        var qty_<?=$item[item_id]?> = parseFloat($('#qty_<?=$item[item_id]?>').val()) || 0;
                                                        $('#total_amt').val((unit_price * qty_<?=$item[item_id]?>).toFixed(2));
                                                    });
                                                });
                                            </script>
                     </table></form>
