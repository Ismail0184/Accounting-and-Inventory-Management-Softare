...
$res="select a.sub_ledger_id,a.sub_ledger_id,a.sub_ledger, (select ledger_name from accounts_ledger where ledger_id=a.ledger_id) as ledger_name, c.group_name,(select COUNT(ledger_id) from journal where ledger_id=a.sub_ledger_id) as has_transactions,s.section_name as branch,
IF(a.status=1, 'Active',IF(a.status='SUSPENDED','SUSPENDED','Inactive')) as status
FROM sub_ledger a,accounts_ledger b,ledger_group c,company s where
a.sub_ledger_id=b.ledger_id and b.ledger_group_id=c.group_id and
    ...
    <form id="form2" name="form2" class="form-horizontal form-label-left" method="post" style="font-size: 11px">
        ...
        <div class="form-group" style="width: 100%">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <select class="select2_single form-control" required name="under" id="under" style="width: 100%; font-size: 12px">
                    <option value=""></option>
                    <?=foreign_relation('accounts_ledger', 'ledger_id', 'CONCAT(ledger_id," : ", ledger_name)', ($POST_under>0) ? $POST_under : $ledger_id, '1','order by ledger_id,ledger_name'); ?>
                </select>
            </div>
        </div>
        <?php if($unique_GET):  ?>
        ...
        <?php else : ?>
            <div class="form-group" style="margin-left:40%">
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <button type="submit" name="record" id="record"  style="font-size:12px" class="btn btn-primary">Add New</button></div></div> <?php endif; ?></form></div></div></div><?php if(!isset($unique_GET)): ?></div><?php endif; ?>
